...
# kustomization.yaml in overlays/production
---
kind: Kustomization
resources:
  - ../../base/
...
```
**resources**
- listet auf welche Kubernetes-Manifeste in den finalen Output eingebunden werden sollen – der Einstiegspunkt dafür, was Kustomize überhaupt "einsammelt"
- Möglichkeit 1: direktes Manifest: deployment.yaml (wird 1:1 im Output übernommen)
- Möglichkeit 2: Pfad zu Verteichnis: database (erwartet eigene kustomization.yaml)

**patches**
- gezielte Änderungen einer bestehenden Ressource vornehmen (z.B. Änderung der replicas)
- target: auf welche Ressource sich das auswirken soll
---
...
oc process {template} --param-file=params.env | oc apply -f - # Parameter werden in file bereitsgestellt, oc apply über STDIN
oc process roster-template \                                  # Vergleich Cluster vs. gerendertes Template
--param-file /home/student/DO280/labs/packaged-templates/roster-parameters.env \
| oc diff -f -
```
## HELM-Charts ##
...
0. oauth identity provider anpassen
1. htpasswd lokal generieren
2. User hinzufügen 
3. Secret anlegen 
4. htpasswd lokal extrahieren und ändern 
5. secret aktualisieren (über set data)

💡**Wichtig**
- secret im NS openshift-config speichern 
- pods von oauth sind im NS openshift-authentication (Neustart dauert ein paar Minuten)
...
htpasswd -c -b -B ./file sascha 'passwrd'                # neue (-c) htpasswd file -B bcrypt very secure -b pass von c.line
                                                         # Eingabe in ' ' bei Sonderzeichen!
...
htpasswd -n -b dba redhat                                # nur Ausgabe Konsole zum Testen, kein Ändern der htpasswd
...
spec:
  ...
  - name: my_htpasswd_provider 
    ...
    htpasswd:
      fileData:
        name: htpass-secret # wichtig: Name muss dem generierten Secret entsprechen
```
---
...
oc adm groups add-users dev-group developer                            # (mehrere) User einer Gruppe zuordnen 
```
...
+--------+                                                +-----------+
                     ...
                     Router schaut nur auf SNI (Server Name Indication)
...
**Route ohne Verschlüsselung**
```bash
oc expose svc todo-http \                                         
--hostname todo-http.apps.ocp4.example.com
```

**Route mit Edge-Verschlüsselung**
- Zertifikat ist von Openshift
```bash
...
oc set volumes deployment/todo-https                               # Secret als Volume im Pod gemounted
 
oc create route passthrough todo-https \                           # Route wird erstellt
--service todo-https --port 8443 \
--hostname todo-https.apps.ocp4.example.com

```
**Debuggen von NW-Verbindungen**
```bash
oc debug -t deployment/todo-https \                                # debug pod öffnen mit curl
--image registry.lab.example.com:8443/ubi9/ubi

curl -s -k https://172.30.75.204:8443 | head -n5                   # passtrough testen
```


## Network Policies

**Beispiel einer NW-Policy**
```yaml
    ...
    - from:
      - namespaceSelector:               # Regel -> welche Namespaces
        ...
        podSelector:                     # Regel -> welche Pods
          matchLabels:
            deployment: sample-app       #  Welche Pods
      ports:
      - port: 8080
        protocol: TCP
```

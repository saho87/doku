...
oc get pods/pod -o jsonpath='{.metadata.annotations}' \ 
  jq '."openshift.io/scc"'
oc get pod podname -o yaml | \                       # Check: erfüllt Deployment default SCC Eigenschaften?
...
```
- oc adm policy: für Cluster-Admins gedacht (deckt cluster als auch projektweite Berechtigungen ab)
